...
export default function WhgcBookingManagePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      try {
        const semRes = await fetch(`/api/seminars/${foundSeminarId}?tenant=${TENANT}`);
  ...
  async function handleCancel() {
    ...
    try {
      const res = await fetch("/api/bookings", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ seminar_id: seminarId, id: rid, tenant: TENANT }),
      });
      if (!res.ok) {
        const err = await res.json();
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto" aria-describedby="whgc-manage-booking-desc">
          <DialogDescription id="whgc-manage-booking-desc" className="sr-only">
