...
export async function POST(request: NextRequest) {
  try {
    ...
    // 事後アンケート回答済チェック (I列 = index 8)
    if (reservation.values[8] === "TRUE") {
      return NextResponse.json({ error: "事後アンケートは回答済みです" }, { status: 400 });
    }
    ...
    await appendRow(spreadsheetId, "事後アンケート", row);

    // 予約情報の事後アンケート回答済フラグを更新 (I列 = index 8)
    await updateCell(spreadsheetId, "予約情報", reservation.rowIndex, 8, "TRUE");
